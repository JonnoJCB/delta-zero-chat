# app.py
# --------------------------------------------------------------
# Δ-Zero Chat – Lightweight Version (No Model Timeout)
# by JCB
# --------------------------------------------------------------
...
# ============================================================== #
def load_knowledge():
    """Load all text lines from knowledge files."""
    knowledge = []
...
class DeltaAgent:
    ...
    def generate_response(self, user_input, slot, mood=None):
        """Generate a simple response based on knowledge or fallback."""
        response = ""

        # Try to find relevant line from knowledge
        if self.knowledge:
            matches = [k for k in self.knowledge if any(word.lower() in k.lower() for word in user_input.split())]
            if matches:
        ...
        if random.random() < 0.3:
            response += " " + random.choice(["Tell me more.", "What do you think?", "Fascinating!"])
...
st.title("Δ-Zero Chat – Lightweight Adaptive AI 🎬")
st.markdown("<sub>by JCB – simple, adaptive and movie-smart</sub>", unsafe_allow_html=True)

# Initialize agent
if "agent" not in st.session_state:
    with st.spinner("Loading Δ-Zero..."):
...
if agent.knowledge:
    st.sidebar.success(f"Loaded {len(agent.knowledge)} movie facts!")
...
# Chat input
if user_input := st.chat_input("Ask me about movies or anything…"):
